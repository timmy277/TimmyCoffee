                ...
                <div className='grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 place-items-center gap-5'>
                    {/* card section */}
                    {
                        ProductsData.map((data) => (
                            <div data-aos="fade-up" data-aos-delay={data.aosDelay} key={data.id} className='space-y-3 group transition'>
                                <img src={data.img} alt="" className='h-[220px] w-[150px] object-cover rounded-md group-hover:scale-110 transition duration-300'/>
                                <div>
                                    <h3 className="font-semibold">{data.title}</h3>
                                    <div className='flex flex-col justify-center items-center absolute top-0 -right-11 group-hover:right-0 group-hover:top-0 opacity-0 group-hover:opacity-100 transition-all duration-300 gap-y-1'>
                                        <button className='bg-transparent hover:scale-150'>
